...
public class GlobalControllerAdvice {
    ...
    public ResponseEntity<?> handleIllegalArgumentException(IllegalArgumentException e) {
        ...
        return ResponseEntity.status(ErrorCode.DATABASE_ERROR.getStatus()).body(ResponseDto.error(ErrorCode.DATABASE_ERROR));
    }
    ...
    public ResponseEntity<?> handleEmptyResultDataAccessException(EmptyResultDataAccessException e) {
        ...
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(ResponseDto.error(ErrorCode.USER_NOT_FOUND));
    }

    @ExceptionHandler(DataIntegrityViolationException.class)
    public ResponseEntity<?> handleDataIntegrityViolationException(DataIntegrityViolationException e) {
        log.error("데이터 무결성 위반: {}", e.toString());
        return ResponseEntity.status(HttpStatus.CONFLICT).body(ResponseDto.error(ErrorCode.DATABASE_ERROR));
    }

    @ExceptionHandler({HttpMessageNotReadableException.class, MethodArgumentTypeMismatchException.class})
    public ResponseEntity<?> handleMessageNotReadableException(Exception e) {
        log.error("메시지 파싱 오류: {}", e.toString());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ResponseDto.error(ErrorCode.VALIDATION_ERROR));
    }
    ...
    public ResponseEntity<?> handleAuthenticationException(AuthenticationException e) {
        ...
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(ResponseDto.error(ErrorCode.INVALID_TOKEN));
    }

    @ExceptionHandler(AccessDeniedException.class)
    public ResponseEntity<?> handleAccessDeniedException(AccessDeniedException e) {
        log.error("접근 거부: {}", e.toString());
        return ResponseEntity.status(HttpStatus.FORBIDDEN).body(ResponseDto.error(ErrorCode.INVALID_TOKEN));
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<?> handleUnexpectedException(Exception e) {
        log.error("예상치 못한 오류 발생: ", e);
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(ResponseDto.error(ErrorCode.DATABASE_ERROR));
    }
}